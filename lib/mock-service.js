...
const dashboard = (date) => {
  ...
  const tiktokScannedCount = scannedRows.filter((p) => p.awb.startsWith('795')).length;
  const tiktokSurplusCount = surplusRows.filter((p) => p.awb.startsWith('795')).length;
  ...
  return {
    ...
    surplus_awbs: surplusRows.filter((p) => !p.awb.startsWith('795')).map((p) => p.awb),
    tiktok_surplus_awbs: surplusRows.filter((p) => p.awb.startsWith('795')).map((p) => p.awb),
