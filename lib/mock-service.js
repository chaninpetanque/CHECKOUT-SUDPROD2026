...
const dashboard = (date) => {
  ...
  const tiktokScanned = scannedRows.filter((p) => p.awb.startsWith('795')).length
    + surplusRows.filter((p) => p.awb.startsWith('795')).length;
  return {
    ...
    total_scanned: scannedRows.length + surplusRows.length,
